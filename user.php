...
$uid=$_GET['uid'];
$sql="select * from all_info where uid='$uid';";
$result=$conn->query($sql);
$row=$result->fetch_assoc();
$email=$row['email'];
$_SESSION['u_email']=$email;
if($_SESSION['u_email']==$_SESSION['email'])
{
   ...
   $email = $_SESSION['u_email']   ; 
       ...
       $sql=" SELECT count(*) FROM user_info.q_bank where q_uploader='$email';";
        ...
        $_SESSION['ac_count']=$row['count(*)'];
         ...
         $sql=" SELECT * FROM user_info.all_info where email='$email';";
        $result=$conn->query($sql);
        $row=$result->fetch_assoc();
        
        $_SESSION['cl_fname']=$row['fname'];
        $_SESSION['cl_lname']=$row['lname'];
        ...
        $table_name="table".$_SESSION['u_email']."following";
         $sql=" SELECT count(*) FROM user_info.`$table_name` ;";
        $result=$conn->query($sql);
        $row=$result->fetch_assoc();
    ...
    <div class="user_name">
        ...
        <p style="text-align: center"><?php echo $_SESSION['cl_fname']." ".$_SESSION['cl_lname']; ?></p>
        

        <?php if($_SESSION['linked']==0) { ?>
        <button id="follow" style="display:block;" title="click to follow">Follow   <i class="fa fa-plus-circle"></i> </button>
        <?php }  if($_SESSION['linked']==1) {?>
        <button id="unfollow" style="display:block;" title="click to unfollow">Following <i class="fa fa-check-circle"></i></button> <?php } ?> 
        
        <script>
        
        $(document).ready(function(){
           $("#follow").click(function(){
              $.post("b_follow_user.php",
                   {
