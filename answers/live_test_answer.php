...
$_SESSION['this_file']=$_SESSION['cur_file']."?qid=q2008101883308439";
...
<!upload-------------clicks-----------------end-------------------------here>   
   ...
   // echo $vqid;
     global $data_set;
    /*$sql="select * from doubtmvu_user_info.q_bank;";
    $result=$conn->query($sql);
    ...
    setsql("select * from doubtmvu_user_info.q_bank where qid='$vqid';");
      global  $i;
      $i=0;
        jump:
...
$_SESSION['t_exam']=$data_set[$i][5];
 ...
 <div class="rating_box">
      ...
      <?php 
                ...
                $sql="select avg(u_popularity), avg(u_lengthy), avg(u_thinking), avg(u_memory_based) from doubtmvu_user_info.user_qna_rating where qid='$vqid' ;";
                $result=$conn->query($sql);
                $row= $result->fetch_assoc();
        ...
        <?php
        $count = "select count(*) from doubtmvu_user_info.a_bank where a_qid='$vqid' and a_certify='0';";
       $result=$conn->query($count);
        ...
        global $c_count;
        $count = "select count(*) from doubtmvu_user_info.a_bank where a_qid='$vqid' and a_certify='1';";
       $result=$conn->query($count);
        $row=$result->fetch_assoc();
        $c_count=$row['count(*)'];
        ?>
...
hitback_sql("update doubtmvu_user_info.q_bank set q_visit= q_visit+1 where qid='$vqid';",$conn);
...
$all_count=$c_count+$uc_count;   
    ...
    a_setsql("select * from doubtmvu_user_info.a_bank where a_qid='$vqid';");
      global  $i;
      $i=0;
        a_jump:
...
{?>
       ...
       $count = "select count(*) from doubtmvu_user_info.a_bank where a_qid='$vqid';";
       $result=$conn->query($count);
        $row=$result->fetch_assoc();
        $count=$row['count(*)'];
        if($i<$count)
        ...
        <div class="rating_box">
            ...
            if($_SESSION['email']!="guest@doubtcool.com")
            {
                ...
                $email=$_SESSION['email'];
                $sql="select * from doubtmvu_user_info.user_qna_rating where email='$email' and qid='$vqid' ;";
                $result=$conn->query($sql);
                $row= $result->fetch_assoc();
